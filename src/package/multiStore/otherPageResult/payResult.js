...
export default class PayResult extends Component {
  ...
  getOrderDetail = () => {
    ...
    if (orderType === 'SCAN_CODE') {
      navToPage(`/package/multiStore/scanningOrder/scanningOrder?orderSn=${orderSn}&enterpriseGuid=${enterpriseGuid}&wxtoken=${wxtoken}&openId=${openId}`)
      return
    }
    Taro.redirectTo({ url: `/pages/orderDetail/orderDetail?orderSn=${this.$router.params.orderSn}&id=${this.$router.params.id}&enterpriseGuid=${enterpriseGuid}&wxtoken=${wxtoken}&openId=${openId}` })
  }
