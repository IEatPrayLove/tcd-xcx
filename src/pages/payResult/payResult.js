...
export default class PayResult extends Component {
  ...
  getOrderDetail = () => {
    const { type: orderType, orderSn } = this.$router.params
    if (!orderSn) {
      showToast('订单编码未知')
      return
    }
    ...
    Taro.redirectTo({ url: `/pages/orderDetail/orderDetail?orderSn=${this.$router.params.orderSn}&id=${this.$router.params.id}` })
  }
